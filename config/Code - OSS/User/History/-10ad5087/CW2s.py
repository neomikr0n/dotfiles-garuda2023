...
def parse_wpctl_status():
    ...
    lines = output.replace("├", "").replace("─", "").replace("│", "").replace("└", "").replace("Easy Effects Sink ", "").replace("Analog Stereo", "").replace("D30", "🎧 Beyerdynamic").replace("", "").replace("Analog Stereo", "").splitlines()
