...
def parse_wpctl_status():
    ...
    for index, sink in enumerate(sinks):
        if sink.startswith("*"):
            sinks[index] = sink.strip().replace("*", "").strip() + " ___active___ "
...
selected_sink_name = subprocess.check_output(f"echo '{output}' | wofi --show=dmenu --hide-scroll --allow-markup --define=hide_search=true --location=top_right --width=311 --height=250 --xoffset=-660 --yoffset=11", shell=True, encoding='utf-8').strip()
sinks = parse_wpctl_status()
