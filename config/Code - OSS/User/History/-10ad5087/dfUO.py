...
def parse_wpctl_status():
    ...
    lines = output.replace("├", "").replace("─", "").replace("│", "").replace("└", "").replace("Easy Effects Sink ", "").replace("Analog Stereo", "").replace("D30", "🎧 Beyerdynamic").replace("PCM2704", "🔊 Bocinas").replace("Analog Stereo", "").splitlines()
