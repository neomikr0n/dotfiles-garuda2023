...
def parse_wpctl_status():
    ...
    lines = output.replace("├", "").replace("─", "").replace("│", "").replace("└", "").replace("Easy Effects Sink ", "").replace("Analog Stereo", "").replace("D30", "🎧 Beyerdynamic").replace("PCM2704 16-bit stereo audio DAC", "🔊 Bocinas").replace("Analog Stereo", "").splitlines()
