...
color=$(hyprpicker --no-fancy -a)
dir=~/Pictures/color.png

# RGB value
rgb=$(printf "rgb(%d, %d, %d)" 0x"${color:1:2}" 0x"${color:3:2}" 0x"${color:5:2}")

# this file will remain on $dir
convert -size 100x100 xc:$color $dir

# Send a notification with the color, rgb value and a image of the color
notify-send -i $dir "Hex color $color copied to clipboard" "RGB: $rgb"
